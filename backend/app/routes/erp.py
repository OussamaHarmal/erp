...
def preview_sage_export(
    ...
    return {
        ...
        "filters": {"start_date": start_date, "end_date": end_date, "only_not_exported": only_not_exported},
    }
...
def export_sage_to_drop_folder(
    ...
    invoices = invoice_query(db, start_date=start_date, end_date=end_date, only_not_exported=True).all()
    errors = collect_sage_errors(invoices)
    if errors:
        raise HTTPException(status_code=400, detail={"message": "Validation Sage échouée", "errors": errors})

    drop_dir = os.getenv("SAGE_AUTO_IMPORT_FOLDER", os.getenv("SAGE_EXPORT_DROP_DIR", r"C:\SAGE_AUTO_IMPORT\pending"))
    os.makedirs(drop_dir, exist_ok=True)
    filename = f"SAGE_VENTES_{start_date or 'DEBUT'}_{end_date or 'FIN'}_JJMMAA.txt".replace("-", "")
    ...
    db.commit()
    return {
        "message": "Fichier Sage préparé dans le dossier surveillé. Si SAGE_AUTO_IMPORT_ENABLED=true sur Windows, Sage va l'importer automatiquement.",
        "path": path,
        "filename": filename,
        "date_format": "JJMMAA",
        "automation_note": "Backend Windows + SAGE_AUTO_IMPORT_ENABLED=true = import automatique. Docker/Linux: génération TXT seulement.",
...
def export_sage_excel(
    ...
    headers = [
        "Code journal", "Date pièce", "N° pièce", "N° facture", "Référence", "N° compte général",
        "N° compte tiers", "Libellé écriture", "Date échéance", "Débit", "Crédit"
    ]
    ws.append(headers)
    ...
    ws_control.append([])
    ws_control.append(["N° facture", "Référence", "Client", "Email", "Date pièce", "Échéance", "HT", "TVA", "TTC", "Exportée", "Validation"])
    for inv in invoices:
        profile = inv.client.profile if inv.client and inv.client.profile else None
        client_name = (profile.company_name if profile and profile.company_name else (profile.full_name if profile else inv.client.email if inv.client else "-"))
    ...
    for sheet in wb.worksheets:
        ...
        for col in range(1, sheet.max_column + 1):
            max_len = max(len(str(sheet.cell(row, col).value or "")) for row in range(1, sheet.max_row + 1))
            sheet.column_dimensions[get_column_letter(col)].width = min(max(12, max_len + 3), 36)

    output = io.BytesIO()
    wb.save(output)
    output.seek(0)
    filename = f"SAGE_CANVAS_COMPTABILITE_{start_date or 'DEBUT'}_{end_date or 'FIN'}.xlsx".replace("-", "")
    ...
    db.add(AuditLog(
        ...
        meta={"start_date": start_date, "end_date": end_date, "invoice_count": len(invoices), "errors": errors},
    ))
    db.commit()
    return StreamingResponse(
        output,
...
def sage_export_history(
    ...
    rows = db.query(SageExportBatch).order_by(SageExportBatch.created_at.desc()).limit(limit).all()
    return [
        {
            "id": str(r.id),
            "filename": r.filename,
